...
fn hash_password(password: &str, salt: &str) -> Result<String,&'static str> {
    ...
    argon2::hash_encoded(password.as_bytes(),salt.as_bytes(),&config).map_err(|_| "Error hashing password")
}
...
impl User {
    ...
    pub fn authenticate(pool: &DBPool, p_username: &str, p_password: &str) -> DBResult<User> {
        use crate::schema::users::dsl::*;
        let mut conn =  pool.get()?;
        let hashed_pwd = hash_password(p_password,p_username).map_err(|err|DBError::InternalErr(err.to_string()))?;
        let user = users.filter(username.eq(p_username))
                        .filter(password_hash.eq(hashed_pwd)).first::<User>(&mut conn)?;
        Ok(user)
    }
    ...
    fn delete(pool: &DBPool, p_id: i32) -> DBResult<()> {
        use crate::schema::users::dsl::*;
        let mut conn =  pool.get()?;
        diesel::delete(users.filter(id.eq(p_id)))
            .execute(&mut conn)?;
        Ok(())
    }
}
